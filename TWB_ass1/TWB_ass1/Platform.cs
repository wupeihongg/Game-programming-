...
namespace TWB_ass1
{
    public class Platform : BasicModel
    {
        ...
        int boxIndex;
        bool ground = false;
        public List<BoundingBox> platformBoxes = new List<BoundingBox>();

        public Platform(Model model, GraphicsDevice device, Vector3 pos)
            : base(model)
        {
            scale = Matrix.CreateScale(3f, 0.2f, 3f);
            this.device = device;
        ...
        public Platform(Model model, bool ground)
            : base(model)
        {
            this.ground = ground;
        }
        ...
        {
            for (int i = 0; i < platformBoxes.Count() - 1; i++)
            {
                platformBoxes.RemoveAt(i);
                i--;
            }
            ...
            foreach (ModelMesh mesh in model.Meshes)
            {
                ...
                platformBoxes.Add(BuildBoundingBox(mesh, meshTransform));
            }
        }
        ...
        {
            BoundingBox box1 = BoundingBoxes.mapBoxes[boxIndex];
            isColliding = false;
            for (int i = 0; i < BoundingBoxes.playerBoxes.Count() - 1; i++)
            {
                if (box1.Contains(BoundingBoxes.playerBoxes[i]) != ContainmentType.Disjoint)
                {
                    isColliding = true;
                }
            }
        ...
        public override void Draw(GraphicsDevice device, Camera camera)
        {
                ...
                foreach (ModelMesh mesh in model.Meshes)
                {
                    foreach (BasicEffect effect in mesh.Effects)
                    {
                        ...
                        effect.AmbientLightColor = new Vector3(0.6f, 0.1f, 0.1f);
                        effect.EmissiveColor = new Vector3(0.1f, 0.2f, 0.2f);
                    } 
                    if (!ground)
                        mesh.Draw();
                }
